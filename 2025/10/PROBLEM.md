...
number of indicator lights shown, but its indicator lights are all **initially
off**.
...
button, each listed indicator light either turns on (if it was off) or turns off
(if it was on). You have to push each button an integer number of times; there's
no such thing as "0.5 presses" (nor can you push a button a negative number of
times).
